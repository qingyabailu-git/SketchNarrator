...
class Config:
    ...
    target_hand_height: int = 0    # 0=按输出短边比例计算；正值保留旧调用方的显式高度
    small_hand_ratio: float = 0.14 # 默认小手高度，占输出画面短边约 12%–16%
    ...
    # ── contour-wipe 上色模式专用 ──
    color_fill: str = "local-brush"  # 上色风格: local-brush 逐对象局部上色 | contour-wipe 旧扫描 | brush 旧轨迹刷
...
class TipOverlay:
    ...
    def visible_ratio(self, canvas_shape: tuple[int, ...], x: int, y: int) -> float:
        """Return the fraction of the overlay rectangle visible at the anchored position."""
        h_canvas, w_canvas = canvas_shape[:2]
        anchor_x = x - self.tip_px
        anchor_y = y - self.tip_py
        visible_w = max(0, min(w_canvas, anchor_x + self.w) - max(0, anchor_x))
        visible_h = max(0, min(h_canvas, anchor_y + self.h) - max(0, anchor_y))
        return (visible_w * visible_h) / max(1, self.w * self.h)

    def stamp(self, canvas: np.ndarray, x: int, y: int, opacity: float = 1.0) -> np.ndarray:
        """让素材的笔尖锚点对齐到落墨点，并支持自然淡入淡出。"""
        ...
        mask_region_3d = self.mask[sy0:sy1, sx0:sx1, None] * opacity
        inv_region_3d = 1.0 - mask_region_3d
        canvas[y0:y1, x0:x1] = (
            region * inv_region_3d + hand_region * mask_region_3d
        ).astype(np.uint8)
        return canvas
...
def trace_8connected(skel: np.ndarray, min_points: int = 8) -> list[list[tuple[int, int]]]:
    ...
    if not points:
        return []
    adj = {p: _skel_neighbors(skel, p) for p in points}
    degrees = {p: len(adj[p]) for p in points}
    starts = (
    ...
    for start in starts:
        for nb in adj.get(start, []):
            ...
            while True:
                path.append(cur)
                next_pt = _choose_next(prev, cur, adj.get(cur, []), visited_edges)
                if next_pt is None:
                    break
...
def _order_skeleton_strokes(strokes: list[list[tuple[float, float]]]) -> list[list[tuple[float, float]]]:
    """
    笔画排序：保持粗粒度的从上到下阅读顺序，同时减少抬笔空走。

    先按笔画高度聚成水平阅读带；每个阅读带内部从当前笔尖选择最近的
    笔画端点，必要时反向笔画。这样不会为了最短路突然跳回上方，也比
    单纯按包围盒左上角排序更像一支笔连续作画。
    """
    ...
    def metrics(stroke):
        ...
        return {
            "top": min(ys), "bottom": max(ys), "left": min(xs),
            "height": max(1.0, max(ys) - min(ys)),
            "length": _stroke_cumulative_length(stroke)[-1],
        }

    meta = {id(stroke): metrics(stroke) for stroke in usable}
    heights = sorted(item["height"] for item in meta.values())
    median_height = heights[len(heights) // 2]
    band_gap = max(12.0, min(48.0, median_height * 0.65))
    bands: list[list[list[tuple[float, float]]]] = []
    for stroke in sorted(usable, key=lambda item: (meta[id(item)]["top"], meta[id(item)]["left"], -meta[id(item)]["length"])):
        info = meta[id(stroke)]
        if not bands:
            bands.append([stroke])
            continue
        band_top = min(meta[id(item)]["top"] for item in bands[-1])
        if info["top"] <= band_top + band_gap:
    ...
    for band in bands:
        ...
        while remaining:
            if tail is None:
                chosen = min(remaining, key=lambda item: (meta[id(item)]["left"], meta[id(item)]["top"], -meta[id(item)]["length"]))
                reverse = False
            else:
                choices = []
                for item in remaining:
                    head_distance = math.hypot(item[0][0] - tail[0], item[0][1] - tail[1])
                    end_distance = math.hypot(item[-1][0] - tail[0], item[-1][1] - tail[1])
                    choices.append((min(head_distance, end_distance), meta[id(item)]["left"], meta[id(item)]["top"], -meta[id(item)]["length"], item, end_distance < head_distance))
                _, _, _, _, chosen, reverse = min(choices, key=lambda value: value[:4])
...
def _semantic_stroke_plan(
    strokes: list[list[tuple[float, float]]],
    ink_mask: np.ndarray,
    allowed_mask: np.ndarray,
) -> list[dict]:
    """Build a deterministic object-first stroke plan without global scan bands.

    Nearby ink fragments are grouped into visual objects.  The largest object
    nearest the annotated region centre is treated as the semantic anchor.  A
    hand draws each object as long structural strokes, supporting details, then
    short texture marks.  The returned records intentionally contain only
    geometry-derived facts so callers can profile and test the plan.
    """
    usable = [list(stroke) for stroke in strokes if len(stroke) >= 2]
    if not usable:
        return []
    active = (ink_mask.astype(bool) & allowed_mask.astype(bool)).astype(np.uint8)
    ys, xs = np.where(allowed_mask)
    if xs.size == 0:
        return []
    diagonal = math.hypot(float(xs.max() - xs.min() + 1), float(ys.max() - ys.min() + 1))
    join_radius = int(np.clip(round(diagonal * 0.012), 5, 19))
    if join_radius % 2 == 0:
        join_radius += 1
    joined = cv2.dilate(
        active,
        cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (join_radius, join_radius)),
        iterations=1,
    )
    count, labels, stats, centroids = cv2.connectedComponentsWithStats(joined, connectivity=8)
    # Sparse relation lines can physically join several real objects into one
    # component.  For dense illustrations, derive density islands as semantic
    # object seeds and assign the connecting strokes to the nearest island.
    if count <= 2 and len(usable) >= 40:
        sigma = max(3.0, diagonal * 0.012)
        density = cv2.GaussianBlur(active.astype(np.float32), (0, 0), sigma)
        ink_density = density[active > 0]
        if ink_density.size:
            threshold = float(np.percentile(ink_density, 50))
            seeds = (density >= threshold).astype(np.uint8)
            seed_count, seed_labels, seed_stats, seed_centroids = cv2.connectedComponentsWithStats(
                seeds, connectivity=8
            )
            minimum_seed_area = max(40, round(diagonal * diagonal * 0.00012))
            keep = [
                label for label in range(1, seed_count)
                if int(seed_stats[label, cv2.CC_STAT_AREA]) >= minimum_seed_area
            ]
            if 2 <= len(keep) <= 12:
                remapped = np.zeros_like(seed_labels)
                compact_stats = np.zeros((len(keep) + 1, seed_stats.shape[1]), dtype=seed_stats.dtype)
                compact_centroids = np.zeros((len(keep) + 1, 2), dtype=seed_centroids.dtype)
                for compact, original in enumerate(keep, start=1):
                    remapped[seed_labels == original] = compact
                    compact_stats[compact] = seed_stats[original]
                    compact_centroids[compact] = seed_centroids[original]
                labels, stats, centroids = remapped, compact_stats, compact_centroids
                count = len(keep) + 1
    region_center = (float(xs.mean()), float(ys.mean()))
    ...
    for label in range(1, count):
        ...
        distance = math.hypot(cx - region_center[0], cy - region_center[1]) / max(1.0, diagonal)
        components[label] = {
            "area": area,
            "center": (cx, cy),
            "anchor_score": area * (1.25 - min(0.75, distance)),
            "strokes": [],
        }

    def component_for(stroke: list[tuple[float, float]]) -> int:
        votes: list[int] = []
        for x, y in stroke[:: max(1, len(stroke) // 24)]:
            xi = max(0, min(labels.shape[1] - 1, int(round(x))))
            yi = max(0, min(labels.shape[0] - 1, int(round(y))))
            label = int(labels[yi, xi])
            if label > 0:
                votes.append(label)
        if votes:
            return max(set(votes), key=lambda value: (votes.count(value), -value))
        mid_x, mid_y = stroke[len(stroke) // 2]
        return min(
            components,
            key=lambda value: math.hypot(
                components[value]["center"][0] - mid_x,
                components[value]["center"][1] - mid_y,
            ),
            default=0,
        )
    ...
    for stroke in usable:
        length = _stroke_cumulative_length(stroke)[-1]
        label = component_for(stroke)
        if label <= 0:
            continue
        components[label]["strokes"].append((stroke, length))
        lengths.append(length)
    components = {key: value for key, value in components.items() if value["strokes"]}
    if not components:
        return [
            {"points": stroke, "component": 0, "role": "detail", "length": _stroke_cumulative_length(stroke)[-1]}
            for stroke in usable
        ]

    texture_limit = max(9.0, diagonal * 0.022)
    remaining_components = set(components)
    component_order: list[int] = []
    first = max(remaining_components, key=lambda value: (components[value]["anchor_score"], -value))
    component_order.append(first)
    remaining_components.remove(first)
    tail_center = components[first]["center"]
    while remaining_components:
        chosen = min(
            remaining_components,
            key=lambda value: (
                math.hypot(components[value]["center"][0] - tail_center[0], components[value]["center"][1] - tail_center[1]),
                -components[value]["area"],
                value,
            ),
        )
        component_order.append(chosen)
        remaining_components.remove(chosen)
        tail_center = components[chosen]["center"]
    ...
    for component_rank, label in enumerate(component_order):
        ...
        component_diagonal = max(
            1.0,
            math.hypot(
                float(stats[label, cv2.CC_STAT_WIDTH]),
        ...
        def semantic_features(item):
            stroke, length = item
            stroke_xs = [point[0] for point in stroke]
            stroke_ys = [point[1] for point in stroke]
            midpoint = stroke[len(stroke) // 2]
            end_gap = math.hypot(stroke[0][0] - stroke[-1][0], stroke[0][1] - stroke[-1][1])
            closed = end_gap <= max(7.0, length * 0.18)
            centrality = 1.0 - min(
                1.0,
                math.hypot(midpoint[0] - component_center[0], midpoint[1] - component_center[1])
                / (component_diagonal * 0.55),
            )
            span = math.hypot(max(stroke_xs) - min(stroke_xs), max(stroke_ys) - min(stroke_ys))
            span_ratio = min(1.0, span / component_diagonal)
            length_ratio = min(1.0, length / component_diagonal)
            # Closed loops and compact central marks often carry identity
            # (face, eye, badge, wheel, bowl, tool head).  Promote them after
            # one stable silhouette anchor instead of drawing every long limb
            # or connector first.
            identity_score = (
                (2.4 if closed else 0.0)
                + centrality * 1.1
                + length_ratio * 0.45
                + (1.0 - span_ratio) * 0.25
            )
            anchor_score = length * (1.0 + span_ratio * 0.35 + centrality * 0.08)
            return {
                "closed": closed,
        ...
        for role, semantic_stage, group in role_groups:
            remaining = list(group)
            while remaining:
                if tail is None:
                    chosen = max(remaining, key=lambda item: (item[1], -item[0][0][0], -item[0][0][1]))
                    reverse = False
                else:
                    choices = []
                    for item in remaining:
                        stroke, length = item
                        head_distance = math.hypot(stroke[0][0] - tail[0], stroke[0][1] - tail[1])
                        end_distance = math.hypot(stroke[-1][0] - tail[0], stroke[-1][1] - tail[1])
                        choices.append((min(head_distance, end_distance), -length, item, end_distance < head_distance))
                    _, _, chosen, reverse = min(choices, key=lambda item: item[:2])
                remaining.remove(chosen)
                points = list(reversed(chosen[0])) if reverse else list(chosen[0])
                planned.append({
                    "points": points,
                    "component": int(label),
                    "componentRank": component_rank,
                    "role": role,
                    "semanticStage": semantic_stage,
                    "length": float(chosen[1]),
                })
                tail = points[-1]
    return planned
...
class StreamBoardRenderer:
    ...
    def __init__(
        ...
        cfg: Config,
        hand_png: Path | None,
        hand_mode: str | bool = "small-hand",
    ) -> None:
        # 保留旧的第四个布尔参数调用方式：旧 True 表示关闭覆盖，旧 False
        # 表示显示笔尖。新的公开契约使用 small-hand/no-hand/full-hand。
        if isinstance(hand_mode, bool):
            hand_mode = "no-hand" if hand_mode else "small-hand"
        if hand_mode == "bare-tip":
            hand_mode = "no-hand"
        if hand_mode not in {"small-hand", "presenter", "no-hand", "full-hand"}:
            raise ValueError(f"不支持的 hand_mode: {hand_mode}")
        if hand_mode == "full-hand" and cfg.target_hand_height <= 0:
            cfg = replace(cfg, target_hand_height=493)
        if hand_mode == "presenter" and cfg.target_hand_height <= 0:
            cfg = replace(cfg, target_hand_height=238)
        ...
        if cfg.target_hand_height <= 0:
            cfg = replace(cfg, target_hand_height=max(1, round(min(self.out_w, self.out_h) * cfg.small_hand_ratio)))
            self.cfg = cfg
    ...
    def _reveal_ink_segment(
        ...
        x1 = min(self.out_w, max(start[0], end[0]) + margin + 1)
        y1 = min(self.out_h, max(start[1], end[1]) + margin + 1)
        if x1 <= x0 or y1 <= y0:
            return
        local_start = (start[0] - x0, start[1] - y0)
        local_end = (end[0] - x0, end[1] - y0)
        seg_local = np.zeros((y1 - y0, x1 - x0), dtype=np.uint8)
        cv2.line(seg_local, local_start, local_end, 255, thickness=thick, lineType=cv2.LINE_AA)
        revealed = (seg_local > 0) & self.ink_pixels[y0:y1, x0:x1]
        target = self.drawn[y0:y1, x0:x1]
        paint = self.ink_paint[y0:y1, x0:x1]
        target[revealed] = paint[revealed]
    ...
    def _snapshot_with_tip(self, px: int, py: int) -> np.ndarray:
        ...
        if self.tip is not None:
            if self.hand_mode != "presenter" or self.tip.visible_ratio(snap.shape, px, py) >= 0.97:
...
def transcode_h264(src: Path, dst: Path) -> Path:
    ...
    if ffmpeg and runtime.get("capabilities", {}).get("libx264"):
        cmd = [
        ...
        if res.returncode == 0:
            ...
            print(f"  H.264 转码完成(FFmpeg/{runtime.get('source')}): {dst}")
            return dst
        print(f"  [warn] ffmpeg 转码失败: {res.stderr.strip()}")
    elif ffmpeg:
        print("  [warn] 当前 FFmpeg 不含 libx264，改用 PyAV 转码")
...
def _parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    p.add_argument(
        "--hand-mode", choices=["small-hand", "presenter", "no-hand", "full-hand"], default="small-hand",
        help="手部覆盖模式：small-hand 默认小手；presenter 通用角色；no-hand 隐藏；full-hand 兼容旧尺寸",
    )
    p.add_argument("--small-hand", dest="hand_mode", action="store_const", const="small-hand", help="small-hand 的兼容快捷参数")
    p.add_argument("--presenter", dest="hand_mode", action="store_const", const="presenter", help="启用通用 Presenter 覆盖层")
    p.add_argument("--no-hand", dest="hand_mode", action="store_const", const="no-hand", help="关闭手部覆盖")
    p.add_argument("--full-hand", dest="hand_mode", action="store_const", const="full-hand", help="启用旧的大手兼容模式")
    p.add_argument(
        "--bare-tip", dest="hand_mode", action="store_const", const="no-hand",
        help="旧兼容别名：过去实际关闭覆盖，不表示小裸笔尖",
    )
    p.add_argument(
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    renderer = StreamBoardRenderer(image_bgr, cfg, pen_png, args.hand_mode)
