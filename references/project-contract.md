...
- `title_card_profile.max_text_chars` 必须为正整数，缺省为 18；配置错误在登记前给出具体字段提示，不以未处理异常退出。
- 含卡片的新视觉计划记录 `layout_plan.title_card_region: [0.025, 0.028, 0.95, 0.112]`，位置与尺寸采用同一归一化坐标系并包含卡片阴影。预设手绘布局从 y=0.16 开始，底部边界不变；自定义区域不得侵入卡片预留区，只报明确冲突，不自动移动用户定义的区域。渲染器按该共享预留区完整等比适配卡片，不裁字、不扩展到手绘区。第二次确认同时展示卡片文案与预留区；旧计划缺少此字段时按旧版读取，不自动迁移或修改已批准画面。
